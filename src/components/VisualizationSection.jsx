...
function VisualizationSection() {
  ...
  return (
    <section className="content-section content-section--magic content-section--visualization" id="visualization">
      ...
      <motion.div
        className="events-slider-wrap magic-frame magic-frame--soft"
        initial="hidden"
        whileInView="visible"
        viewport={viewportOnce}
        variants={panelRevealVariants}
      >
        <div className="magic-scanline magic-scanline--slow" aria-hidden="true" />

        <button
          className="slider-arrow slider-arrow-visual-prev"
          type="button"
          aria-label={'Предыдущий слайд визуализации'}
          onClick={handlePrev}
        >
          <FaArrowLeft />
        </button>
        ...
        <div className="events-slider" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
          <AnimatePresence mode="wait">
            <motion.article
              ...
              className="event-slide event-slide--visual magic-slide"
          ...
          <div className="events-slider__dots" aria-label="Навигация по слайдам визуализации">
            {visualizationSlides.map((slide, index) => (
              <button
                key={slide.title}
                type="button"
                className={`events-slider__dot${index === activeIndex ? ' is-active' : ''}`}
                aria-label={`Перейти к слайду визуализации ${index + 1}`}
        ...
        <button
          className="slider-arrow slider-arrow-visual-next"
          type="button"
          aria-label={'Следующий слайд визуализации'}
          onClick={handleNext}
        >
          <FaArrowRight />
        </button>
